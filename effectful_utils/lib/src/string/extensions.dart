...
extension StringSliceExtension on String {
  ...
  String slice(int start, [int end = -1]) {
    final s = start < 0 ? start + length : start;
    final e = end < 0 ? end + length : end;

    RangeError.checkValidRange(s, e, length);
...
extension StringOrNullExtension on String {
  /// Returns `null` if this string is empty, otherwise returns this string.
...
extension StringUrlCodingExtension on String {
  /// Encodes this string for use in a full URI using [Uri.encodeFull].
  String get urlEncode => Uri.encodeFull(this);

  /// Decodes a string previously encoded for a full URI using [Uri.decodeFull].
